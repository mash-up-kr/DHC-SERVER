...
class PastPastRoutineHistoryRepositoryTest : BaseMongoDBTest() {
    ...
    @Test
    fun `test insert and find missionHistory by id`() =
        runBlocking {
            // Given
            val userId = ObjectId()
            val today = now()
            ...
            // When
            val insertedId = pastRoutineHistoryRepository.insertOne(pastRoutineHistory)
    ...
    @Test
    fun `test find missionHistories by userId`() =
        runBlocking {
            // Given
            ...
            val history3 = PastRoutineHistory(id = null, userId = userId2, date = twoDaysAgo, missions = emptyList())

            // Insert all mission histories
            pastRoutineHistoryRepository.insertOne(history1)
            pastRoutineHistoryRepository.insertOne(history2)
            pastRoutineHistoryRepository.insertOne(history3)
    ...
    @Test
    fun `test find missionHistory by userId and date`() =
        runBlocking {
            // Given
            val userId1 = ObjectId()
            val userId2 = ObjectId()
            ...
            // When
            val user1TodayHistory = pastRoutineHistoryRepository.findByUserIdAndDate(userId1, today)
            val user1YesterdayHistory = pastRoutineHistoryRepository.findByUserIdAndDate(userId1, yesterday)
            val user2TodayHistory = pastRoutineHistoryRepository.findByUserIdAndDate(userId2, today)
            val nonExistingHistory = pastRoutineHistoryRepository.findByUserIdAndDate(userId2, yesterday)
